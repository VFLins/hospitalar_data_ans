...
temp[temp %in% c("Cooperativa Médica", "Medicina De Grupo")] <- "Modalidade cooperativa"
temp[temp %in% c("Seguradora Especializada Em Saúde", 
					  "Filantropia", "Autogestão", NA)] <- "Modalidade não-cooperativa"
...
temp <- mdl1_df$ANO_MES_EVENTO
get_month <- function(x) as.numeric(format(x, "%m"))
temp[get_month(temp) %in% 1:3] <- "Primeiro"
temp[get_month(temp) %in% 4:6] <- "Segundo"
temp[get_month(temp) %in% 7:9] <- "Terceiro"
temp[get_month(temp) %in% 10:12] <- "Quarto"
