...
```{r MDL1--TARGET-VARIABLE}
# Criando variavel target
df$ACIONAMENTO_SEGURO <- df$VL_ITEM_PAGO_FORNECEDOR > 0

# Obtendo informações sobre o balanceamento dos dados
QTD_ACIONAMENTOS <- sum(df$ACIONAMENTO_SEGURO)
QTD_CASOS <- nrow(df)
```
...
negative_sample_index <- sample(x=df_index[!df$ACIONAMENTO_SEGURO], size=QTD_ACIONAMENTOS)
positive_index <- df_index[df$ACIONAMENTO_SEGURO]
# Criando novo dataframe com valores de "ACIONAMENTO_SEGURO" balanceados
mdl1_df <- df[c(negative_sample_index, positive_index),]
```
