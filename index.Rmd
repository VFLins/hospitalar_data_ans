...
O *Precision-Recall AUC* (PRAUC) é independente do limiar escolhido, e será usado para avaliar a capacidade preditiva geral de cada modelo, já o

Agora que já temos os dados da validação cruzada, podemos obter as métricas de desempenho de cada um baseado nos critérios que acabamos de definir.
...
for (i in seq_along(cvdata)) {
	...
	f2score <- FBeta_Score(
		cvdata[[i]]$y_actual, 
		{cvdata[[i]]$y_predict>=.5}, 
		beta=2
	)
	
	print(paste(nm, "PRAUC:", prauc))
	print(paste(nm, "F-2:", f2score)	)
}
```
