...
O melhor resultado que obtivemos foi na regressão logística, agora que conseguimos escolher o modelo, vamos definir o limiar de probabilidade que separa o que será classificado como "Acionamento do sinistro" e o que será "Não acionamento do sinistro". Para isso, vamos obter a métrica `F2-score` para um conjunto de limiares possíveis, para economizar tempo de computação, vamos usar 19 possíveis limiares de 5% à 95%.

```{r}
select_threshold(temp$y_actual, temp$y_predict, plot_=TRUE)
```
