...
```{r CV-LOAD, include=FALSE}
cvdata <- readRDS("cv_data.rds")
```

Agora que temos os dados de erros dos modelos, podemos obter métricas customizadas, não precisamos nos limitar ao que os pacotes do R oferecem por padrão. Para o propósito deste desafio, vamos usar as métricas:

-   **Precision-Recall AUC**

-   **Uma métrica customizada**

Como estamos usando um modelo em que as previsões são desbalanceadas, e em que o falso negativo (erro tipo 2) oferece mais custos de negócio que os falsos positivos (erro tipo 1), nossas métricas de erro devem priorizar a capacidade do modelo de prever corretamente os casos positivos e minimizar os erros tipo 2. Por este motivo, ambas as métricas que escolhemos incorporam a *Taxa de Positivo Verdadeiro* (ou *recall*) e o *Valor Preditivo Positivo* (ou *Precision*).
...
O *Precision-Recall AUC* (PRAUC) é independente do limiar escolhido, e será usado para avaliar a capacidade preditiva geral de cada modelo, já o *F-2 Socore* será usado para definir o limiar ideal, uma vez que o modelo foi escolhido. Agora que já temos os dados da validação cruzada, podemos obter as métricas de desempenho de cada um baseado nos critérios que acabamos de definir.

```{r CV-FIRST-RESULTS}
for (i in seq_along(cvdata)) {
	nm <- names(cvdata[i])[1]
	prauc <- AUC(cvdata[[i]]$y_predict, cvdata[[i]]$y_actual)
	print(paste(nm, "PRAUC:", prauc))
}
```

O melhor resultado que obtivemos foi na regressão logística, agora que conseguimos escolher o modelo, vamos definir o limiar de probabilidade que separa o que será classificado como "Acionamento do sinistro" e o que será "Não acionamento do sinistro". Para isso vamos usar uma métrica customizada $M$, que consiste na diferença entre recall e a taxa de falso negativo:

$$
M = Recall \times (1+Precision) \times FNR^3
$$

Todos os valores que compõem a métrica variam de 0 a 1, os dois primeiros são indicadores de bom desempenho do modelo, enquanto que o último é o termo de penalidade, que penaliza pela Taxa de Falsos Negativos (erro mais indesejado, como discutido anteriormente).

Adicionar `Precision` na fórmula também ajuda a controlar a métrica para os falsos positivos, que não é o erro mais caro de cometer, mas um modelo bem ajustado deve otimizar a quantidade qualquer tipo de erros.

Para esta métrica, é esperado um valor sempre maior que zero, e que quanto maior, melhor o desempenho de nosso modelo em prever corretamente os casos positivos minimizando os erros adequadamente. Vamos obter o valor da métrica para diferentes limiares de probabilidade e escolher o primeiro ponto que maximiza o valor da métrica
...
Agora chegou a hora de descobrir como este modelo desempenha "no mundo real", vamos usar o recorte de dados que criamos anteriormente.

```{r}
y <- adf$sinistro
y_pred <- predict(mdl_rf, adf, type="prob")[, "TRUE"]
confusion_matrix({y_pred >= .94}, y)
```
