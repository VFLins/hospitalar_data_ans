...
pred_df <- data.frame(
	...
	logreg=predict(mdl1, df),
	naiveb=predict(mdl1_nb, df, type="prob"),
	rndfst=predict(mdl1_rf, df)
)
```

E em seguida obter as métricas para cada limiar possível:

```{r}
thresholds <- c(1:99)/100
logreg_fpr <- c(); logreg_tpr <- c()
naiveb_fpr <- c(); naiveb_tpr <- c()
rndfst_fpr <- c(); rndfst_tpr <- c()
for (i in thresholds) {
	
	logreg_fpr <- c(logreg_fpr, (pred_df$logreg >= i))
}
```
