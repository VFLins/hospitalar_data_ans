...
E em seguida obter as métricas para cada limiar possível, e elaborar uma visualização das *Precision-Recall Cruves* (PRC):

```{r plot--ROC_MODELS}
temp <- prc_curve(
	pred_df[c("Regressao.Logistica", "Naive.Bayes", "Random.Forest")], 
	pred_df$y
)

Diagonalx = seq(0, 1, length.out=99)
Diagonaly = seq(1, 0, length.out=99)
p <- ggplot(temp) + 
	geom_line(
		aes(
			x=Regressao.Logistica_tpr, 
			y=Regressao.Logistica_precision
		), color="red4", size=1.2) +
	geom_line(
		aes(
			x=Naive.Bayes_tpr, 
			y=Naive.Bayes_precision
		), color="#b66868", size=1.2) +
	geom_line(
		aes(
			x=Random.Forest_tpr, 
			y=Random.Forest_precision
		), color="orange2", size=1.2) +
	#geom_line(aes(x=Diagonalx, y=Diagonaly), color="gray30", linetype="dotted") +
	ylab("Precision") + xlab("Recall") + ggtitle("PRC para cada modelo") +
	my_ggtheme()
ggplotly(p)
```

O resultado é tão similar para os modelos que eu tive que usar pontos na visualização em vez de linhas para ter alguma maneira de diferenciar as curvas, mantendo o mesmo arranjo de variáveis em diferentes modelos, o resultado final deve variar muito pouco. Dito isto, vamos continuar com mais uma rodada de testes, para encontrar o melhor modelo para este propósito.
...
Como estamos usando um modelo em que as previsões são desbalanceadas, e em que o falso negativo (erro tipo 1) oferece mais custos de negócio que os falsos negativos (erro tipo 2), nossas métricas de erro deve priorizar a capacidade do modelo de prever corretamente os casos positivos e minimizar os erros tipo 1. Por este motivo, ambas as métricas que escolhemos incorporam a *Taxa de Positivo Verdadeiro* (ou *recall*) e o *Valor Preditivo Positivo* (ou *Precision*).

O valor previsto pelos modelos no caso de classificação é interpretado como a probablidade de ocorrência do caso postivo (acionamento do sinistro), há um limiar de 50% definido por padrão na maioria dos casos de previsão, onde a probabilidade maior que este valor é entendida como uma previsão de que o caso positivo acontecerá. Os modelos de classificação podem ser ajustados para capturar mais os casos positivos ou os casos negativos quando alteramos o limiar da probablidade prevista.
...
```{r}
#mdl_predict <- predict.glm(mdl, mdl_df)
```
