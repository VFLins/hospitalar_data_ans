...
form1 <- ACIONAMENTO_DO_SEGURO ~ trimestre + idade + modalidade * regiao
mdl_lg <- glm(form1, data=mdl1_df)
summary(mdl_lg)
```
...
form2 <- factor(ACIONAMENTO_DO_SEGURO) ~ trimestre + idade + modalidade * regiao
mdl_nb <- naive_bayes(form1)
mdl_rf <- randomForest(form2, data=mdl1_df)
```
...
```{r mirror--PREDICTIONS_TABLE, warning=FALSE}
pred_df <- data.frame(
	y=df$ACIONAMENTO_DO_SEGURO,
	Regressao.Logistica=predict(mdl_lg, df),
	Naive.Bayes=predict(mdl_nb, df, type="prob")[, "TRUE"],
	Random.Forest=predict(mdl_rf, df, type="prob")[, "TRUE"]
)
```

E em seguida obter as métricas para cada limiar possível, e elaborar uma visualização:
...
Diagonal = seq(0, 1, length.out=99)
p <- ggplot(temp) + 
	geom_point(aes(x=Regressao.Logistica_fpr, y=Regressao.Logistica_tpr), color="red4", size=2) +
	geom_point(aes(x=Naive.Bayes_fpr, y=Naive.Bayes_tpr), color="#b66868", size=2) +
	geom_point(aes(x=Random.Forest_fpr, y=Random.Forest_tpr), color="tomato2", size=2) +
	geom_line(aes(x=Diagonal, y=Diagonal), color="gray30", linetype="dotted") +
	ylab("True positive rate") + xlab(NULL) + ggtitle("Curvas ROC para cada modelo") +
	my_ggtheme()
ggplotly(p)
```
...
O método de Monte Carlo foi escolhido no lugar do K-Fold, pois este apresenta menor variância para as métricas, o que será um bom equiliíbrio, uma vez que já estamos controlando os modelos para viés. Assim estamos buscando um equilíbrio no *trade-off* conhecido entre viés e variância.

```{r CROSS-VALIDATION, eval=FALSE}
cvdata <- mccv_data(
	formulas=c(form1, form1, form2),
	models=c(LogReg=glm, NBayes=naive_bayes, RndFst=randomForest),
	types=c("response", "prob", "prob"),
	dataset=df,
	balanced_for="ACIONAMENTO_DO_SEGURO",
	n_repeats=14)
```
