...
A taxa de falso negativo pode variar entre 0 e 1, assim como o recall. A diferença é que conforme aumentamos o limiar, estes se deslocam de origens diferentes à destinos diferentes. 

O mais comum é que esta métrica varie de um valor negativo ($FNR > Recall$) para um valor positivo ($Recall > FNR$), embora observemos uma oscilação neste sentido, a métrica que obtemos não chega nunca num valor positivo:
...
Isto significa que o Recall nunca cresce o suficiente para superar a Taxa de Falso Negativo, o que resulta num desempenho subótimo para um modelo em produção. O que é relativamente aceitável neste contexto, mas um modelo eficaz dependeria de um trabalho mais extenso de obtenção e tratamento de variáveis exógenas.
