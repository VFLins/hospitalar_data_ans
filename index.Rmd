...
$$
M_L = \frac{TP_L}{(TP_L+FP_L)} - \frac{FN_L}{(FN_L+TP_L)}
$$
...
Esperamos que nosso modelo seja capaz de maximizar a proporção de valores positivos, assim nós vamos obter o valor des ta métrica para cada limiar $L$, obtemos um valor de $M_L$, que pode oscilar entre 1 e -1, e quanto maior o valor obtido, melhor é o desempenho do modelo nas restrições estabelecidas.
...
for (i in seq_along(cvdata)) {
	nm <- names(cvdata[i])[1]
	prauc <- avg_metric(cvdata[[i]]$y_actual, cvdata[[i]]$y_predict)
	print(paste(nm, "média da métrica:", prauc))
}
```
